...
import { Brain, TrendingUp, BarChart3, Activity, Zap, AlertTriangle, RefreshCw } from 'lucide-react';
...
const CNN1D = () => {
  ...
  const loadModelData = async () => {
    ...
    try {
      ...
      // Get model metrics
      const metrics = await mlBotService.getModelMetrics('cnn1d');
      if (metrics) {
        setModelData(prev => ({
          ...prev,
      ...
      const prediction = await mlBotService.getModelPrediction('cnn1d', selectedSymbol);
      if (prediction) {
        setModelData(prev => ({
          ...prev,
      ...
      console.error('Error loading CNN1D data:', error);
  ...
  const handleSymbolChange = async (symbol) => {
    ...
    const prediction = await mlBotService.getModelPrediction('cnn1d', symbol);
    if (prediction) {
      setModelData(prev => ({
        ...prev,
        currentSignal: prediction.signal || 'No Signal',
  ...
  const handleTrainModel = async () => {
    try {
      setIsLoading(true);
      const result = await mlBotService.trainModel('cnn1d', {
        epochs: 100,
        batch_size: 32,
        learning_rate: 0.001
      });
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <h1 className="text-2xl font-bold text-[var(--text-primary)]">CNN1D Model</h1>
      ...
      {/* Model Analysis */}
      <div className="bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-lg p-6">
        <h2 className="text-lg font-semibold text-[var(--text-primary)] mb-4">CNN1D Analysis</h2>
        <div className="space-y-4">
          <div className="p-4 bg-[var(--bg-tertiary)] rounded-lg">
            <h3 className="font-medium text-[var(--text-primary)] mb-2">Model Architecture</h3>
            <p className="text-[var(--text-secondary)]">
              1D Convolutional Neural Network optimized for time series pattern recognition in financial data. 
              Uses multiple convolutional layers with different kernel sizes to capture various temporal patterns.
            </p>
          </div>
          <div className="p-4 bg-[var(--bg-tertiary)] rounded-lg">
            <h3 className="font-medium text-[var(--text-primary)] mb-2">Key Features</h3>
            <ul className="text-[var(--text-secondary)] space-y-1">
              <li>• Temporal pattern recognition in price movements</li>
              <li>• Multi-scale feature extraction</li>
              <li>• Real-time prediction capabilities</li>
              <li>• Robust to market noise</li>
            </ul>
          </div>
          <div className="p-4 bg-[var(--bg-tertiary)] rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-3 bg-green-500 bg-opacity-10 border border-green-500 border-opacity-20 rounded-lg">
                <p className="text-green-500 font-medium">{modelData.currentSignal}</p>
      ...
      <div className="bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-lg p-6">
        ...
        <div className="h-64 flex items-center justify-center border-2 border-dashed border-[var(--border-primary)] rounded-lg">
          <div className="text-center">
            ...
            <p className="text-[var(--text-secondary)]">
              {isConnected 
                ? 'Real-time performance data from Local Bot' 
                : 'Connect to Local Bot to view performance data'
              }
            </p>
            {!isConnected && (
